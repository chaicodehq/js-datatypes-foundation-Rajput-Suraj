...
export function getThaliStats(thalis) {
  ...
  const totalSum = thalis.reduce((acc, curr) => acc + curr.price, 0);
  const avgPrice = (totalSum / thalis.length).toFixed(2);
...
export function generateThaliReceipt(customerName, thalis) {
  ...
  )}\n---\nTotal: Rs.${thalis.reduce((acc, curr) => acc + curr.price, 0)}\nItems: ${thalis.map((thali) => thali).length}`;
}
